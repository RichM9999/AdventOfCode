...
namespace AdventOfCode.Year2025
{
    class Day6
    {
        ...
        private long Part1()
        {
            ...
            // Use puzzle input by rows
            LoadData(true);

            return SumProblemAnswers();
        }
        ...
        private long Part2()
        {
            ...
            // Use puzzle input by columns
            LoadData(false);

            return SumProblemAnswers();
        }

        private long SumProblemAnswers()
        {
            long sum = 0;

            foreach (var problem in problems)
            {
                if (problem.isMultiplication)
                {
                    // For multipliciation, skip numbers equal to zero and multiply by 1 instead
                    // When loading data by columns, there could be just two or three numbers and the rest will still be initialized to zero, so skip
                    sum += problem.number1 * problem.number2 * (problem.number3 > 0 ? problem.number3 : 1) * (problem.number4 > 0 ? problem.number4 : 1);
                }
                else
                {
                    sum += problem.number1 + problem.number2 + problem.number3 + problem.number4;
                }
            }
        ...
        private void LoadData(bool byRow)
        {
            ...
            if (byRow)
            {
                ...
                var row3Array = row3.Split(' ');
                var row4Array = row4.Split(' ');
                var row5Array = row5.Split(' ');

                // Get numbers from each row plus operation
                for (var r = 0; r < row1Array.Length; r++)
                {
                    problems.Add((long.Parse(row1Array[r]), long.Parse(row2Array[r]), long.Parse(row3Array[r]), long.Parse(row4Array[r]), row5Array[r] == "*"));
                }
            }
            else
            {
                ...
                // From right-to-left...
                for (var r = row1.Length - 1; r >= 0; r--)
                {
                    // Build next number with digits from each row
                    numbers[number] = $"{row1[r]}{row2[r]}{row3[r]}{row4[r]}";
                    number++;

                    // Check if column is left edge of problem, with operation
                    if (row5[r] != ' ')
                    {
                        // Add numbers and operation 
                        problems.Add((long.Parse(numbers[0]), long.Parse(numbers[1]), long.Parse(numbers[2]), long.Parse(numbers[3]), row5[r] == '*'));
                        // Reset number array and number index
                        numbers = ["0", "0", "0", "0"];
                        number = 0;
