namespace AdventOfCode
{
    class Day5
    {
        public void Run()
        {
            ...
            {
                var pages = update.Split(',').ToArray();

                var invalid = false;

                // Check values right to left
                for (var a = pages.Length - 1; a > 1; a--)
                { 
                    var page = pages[a];

                    // Find any values that have a before requirement
                    if (rules.Any(r => r.StartsWith($"{page}|")))
                    {
                        for (var b = 0; b < a; b++)
                        {
                            var before = pages[b];

                            // If there's a value to the left that needs to be after, then it's invalid
                            if (rules.Any(r => r == $"{page}|{before}"))
                            {
                                invalid = true;
                                break;
                            }
                ...
                if (invalid)
                {
                    ...
                    while (swap)
                    {
                        ...
                        for (var a = 0; a < pages.Length - 1; a++)
                        {
                            var after = pages[a];

                            if (rules.Any(r => r.EndsWith($"|{after}")))
                            {
                                for (var b = 0; b < pages.Length; b++) 
                                {
                                    var before = pages[b];

                                    // Found a before after and after..
                                    if ( b > a && rules.Any(r => r == $"{before}|{after}"))
                                    {
                                        var copy = (string[])pages.Clone();

                                        // Move before value before the after
                                        for (var c = 0; c < b; c++)
                                        {
                                            if (c == a)
                                            {
                    ...
                    int.TryParse(pages[pages.Length / 2], out int value);
                    correctedSum += value;
                }
                else
                {
            ...
            Console.WriteLine($"Valid sum of middles: {sum}");
            Console.WriteLine($"Invalid sum of corrected middles: {correctedSum}");
